...
function getMessageType(message: BaseMessage): string {
  ...
  // Fallback to deprecated _getType for backward compatibility
  ...
  return (
    <div className="flex flex-col gap-1 w-full">
      {messages.map((msg, idx) => {
        ...
        return (
          <div
            key={msg.id || `message-${messageTypeLabel}-${idx}`}
            ...
            {"tool_calls" in msg && msg.tool_calls ? (
              <div className="flex flex-col gap-1 items-start w-full">
                {(msg.tool_calls as ToolCall[]).map((tc, idx) => (
                  <ToolCallTable
                    key={tc.id || `tool-call-${tc.name}-${idx}`}
                    toolCall={tc}
                  />
                ))}
              </div>
...
function StateViewRecursive(props: Readonly<StateViewRecursiveProps>) {
    ...
    return (
      <div className="flex flex-row gap-1 items-start justify-start w-full">
        ...
        {valueArray.map((item, idx) => {
          ...
          return (
            <div
              key={`state-view-item-${idx}-${typeof item}`}
...
function HasContentsEllipsis({ onClick }: Readonly<{ onClick?: () => void }>) {
  return (
    <button
      type="button"
      onClick={onClick}
      ...
      className={cn(
        ...
        "-translate-y-[2px] inline-block border-0",
      )}
    >
      {"{...}"}
    </button>
...
export function StateViewObject(props: Readonly<StateViewProps>) {
    ...
    <div className="flex flex-row gap-2 items-start justify-start relative text-sm">
      ...
      >
        <button
          ...
          className="w-5 h-5 flex items-center justify-center hover:bg-gray-100 text-gray-500 hover:text-black rounded-md transition-colors ease-in-out cursor-pointer border-0 bg-transparent"
        >
...
export function StateView({
  ...
  values,
  description,
}: Readonly<StateViewComponentProps>) {
  const [expanded, setExpanded] = useState(false);
