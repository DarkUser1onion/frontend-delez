...
describe("ThreadHistory", () => {
  beforeEach(() => {
    ...
    useThreadsMock.mockReturnValue({
      ...
      threads: [
        makeThread("t-1", "Первый тред", "general"),
        makeThread("t-2", "Второй тред", "entry"),
      ],
  ...
  it("обрабатывает ошибку создания категорийного чата", async () => {
    ...
    fireEvent.click(
      screen.getAllByLabelText("Новый чат в разделе Эксперименты")[0],
    );

    await waitFor(() => {
      expect(errorSpy).toHaveBeenCalledWith(
        "Не удалось создать чат в категории",
        expect.any(Error),
      );
    });
  ...
  it("открывает меню треда, переключает избранное и категорию", async () => {
    ...
    getThreadsMock.mockResolvedValueOnce([
      makeThread("t-1", "Первый тред", "goal"),
    ]);
    render(<ThreadHistory />);
    ...
    fireEvent.click(
      ...
      ).getByText("Убрать из избранного"),
    );
