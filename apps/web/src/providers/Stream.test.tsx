...
vi.mock("@langchain/langgraph-sdk/react-ui", () => ({
  uiMessageReducer: vi.fn((current: unknown[], event: unknown) => [
    ...current,
    event,
  ]),
}));
...
vi.mock("nuqs", () => ({
  useQueryState: vi.fn((key: string, options?: { defaultValue?: unknown }) => {
    const value =
      key in queryStateValues
        ? queryStateValues[key]
...
vi.mock("@/components/icons/langgraph", () => ({
  LangGraphLogoSVG: (props: React.SVGProps<SVGSVGElement>) => (
    <svg {...props} />
  ),
}));
...
describe("StreamProvider", () => {
  ...
  it("shows configuration form when required values are missing and saves submitted values", () => {
    ...
    fireEvent.submit(
      screen.getByRole("button", { name: /Продолжить/i }).closest("form")!,
    );
