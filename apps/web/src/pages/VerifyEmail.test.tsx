...
describe("VerifyEmail", () => {
  ...
  describe("without token (resend form)", () => {
    it("renders the resend verification form when no token", () => {
      ...
      expect(
        screen.getByRole("heading", { name: TEXT.confirmEmailTitle }),
      ).toBeInTheDocument();
      expect(screen.getByLabelText(TEXT.emailLabel)).toBeInTheDocument();
      expect(
        screen.getByRole("button", { name: TEXT.sendEmail }),
      ).toBeInTheDocument();
    });
  ...
  describe("with token", () => {
    it("shows loading state while verifying", () => {
      vi.mocked(authApi.verifyEmail).mockImplementationOnce(
        () => new Promise(() => {}),
      );
    ...
    it("shows success state after email verified", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByRole("heading", { name: TEXT.emailConfirmed }),
        ).toBeInTheDocument();
      });
    ...
    it("stores session token in localStorage on success", async () => {
      ...
      await waitFor(() => {
        expect(globalThis.localStorage.getItem("auth_token")).toBe(
          "my-session-token",
        );
      });
    });
    ...
    it("shows error state when verification fails", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByRole("heading", { name: TEXT.verificationError }),
        ).toBeInTheDocument();
      });
