...
export default function RecordsPage() {
  ...
  const stopNotebookEntryCreation = (): void => setIsCreatingNotebookEntry(false);
  ...
  const persistSelectedNotebook = (): void => {
    ...
    setNotebookDrafts((prev) => ({ ...prev, [selectedNotebook.id]: defaultDraft(selectedNotebook.title) }));
  ...
  const exportSelectedNotebook = (): void => {
    ...
    if (selectedDraft.imageUrl) lines.push(`![image](${selectedDraft.imageUrl})`, "");
  ...
  return (
    <div className="min-h-full bg-[#000019] text-white">
      ...
      <div className="px-8 pb-28">
              ...
              >
                <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    <button type="button" onClick={stopNotebookEntryCreation}
                    ...
                    <button type="button" onClick={persistSelectedNotebook}
                    ...
                    <button type="button" onClick={exportSelectedNotebook}
                      className="inline-flex items-center gap-2 rounded-xl border border-white/20 bg-white/10 px-3 py-2 text-xs text-white transition hover:bg-white/20">
                      <Download size={14} />
                      Экспорт .md
                    </button>
                ...
                <div className="space-y-3">
                  <input value={selectedDraft.title} onChange={(event) => updateSelectedDraft({ title: event.target.value })}
                  ...
                  <textarea ref={textareaRef} rows={10} value={selectedDraft.content}
                  ...
                  <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                    <label className="block">
                      <span className="mb-1 inline-flex items-center gap-1 text-xs text-gray-400"><ImagePlus size={12} />Изображение (URL)</span>
                      <input value={selectedDraft.imageUrl} onChange={(event) => updateSelectedDraft({ imageUrl: event.target.value })}
                        placeholder="https://..."
                    ...
                    <label className="block">
                      <span className="mb-1 inline-flex items-center gap-1 text-xs text-gray-400"><ImagePlus size={12} />GIF (URL)</span>
                      <input value={selectedDraft.gifUrl} onChange={(event) => updateSelectedDraft({ gifUrl: event.target.value })}
                        placeholder="https://..."
                        className="h-10 w-full rounded-lg border border-white/15 bg-[#070b22]/90 px-3 text-xs text-white outline-none placeholder:text-white/40"
                      />
                    </label>
                  ...
                  <div className="flex items-center justify-center gap-2 rounded-xl border border-dashed border-white/20 bg-white/5 py-3 text-xs text-gray-400">
                    <ImagePlus size={14} />Перетащите изображение в эту область для загрузки
                  </div>
                  <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                    <div className="rounded-xl border border-white/10 bg-black/20 p-2">
                      <div className="mb-2 flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          <button type="button" onClick={() => setDrawMode((prev) => !prev)}
                    ...
                    <div className="rounded-xl border border-white/10 bg-black/20 p-3">
                      ...
                      {selectedDraft.imageUrl ? (
                        ...
                        <div className="mb-2 flex h-28 items-center justify-center rounded-lg border border-dashed border-white/10 text-xs text-gray-500">Изображение не выбрано</div>
                      )}
                      {selectedDraft.gifUrl ? (
                        <img src={selectedDraft.gifUrl} alt="notebook gif" className="h-28 w-full rounded-lg object-cover" />
                      ) : (
                        <div className="flex h-28 items-center justify-center rounded-lg border border-dashed border-white/10 text-xs text-gray-500">GIF не выбран</div>
                      )}
                    </div>
                  </div>
                </div>
              ...
              <>
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
                  {[
                    ...
                    <div key={item.label} className="rounded-2xl border p-5" style={{ borderColor: "rgba(255,255,255,0.10)", background: "rgba(255,255,255,0.03)" }}>
        ...
        {activeTab === "ai" ? (
          <div className="rounded-2xl border p-6" style={{ borderColor: "rgba(255,255,255,0.10)", background: "rgba(255,255,255,0.03)" }}>
          ...
          {recordTabs.map((tab) => {
            ...
            return (
              <button key={tab.id} type="button"
                onClick={() => {
                  if (tab.id === "ai") { navigate("/chat"); return; }
                  setActiveTab(tab.id);
                }}
