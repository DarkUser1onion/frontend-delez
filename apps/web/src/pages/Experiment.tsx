...
function parseAggregatedResponse(
  r: Record<string, unknown>,
): ExperimentDetail | null {
  const exp = r.experiment as Record<string, unknown>;
  if (typeof exp?.id !== "string" || typeof exp?.status !== "string")
    return null;
  ...
  return {
    ...
    related_entries: Array.isArray(r.related_entries)
      ? (r.related_entries as RelatedEntry[])
      : [],
    tested_concepts: Array.isArray(r.tested_concepts)
      ? (r.tested_concepts as TestedConcept[])
      : [],
  };
}

function parseFlatResponse(
  r: Record<string, unknown>,
): ExperimentDetail | null {
...
function buildIntensityPath(
  ...
  const points = sorted.map((m, i) => {
    ...
    const y =
      height -
      ((m.intensity_value - minVal) / range) * (height * 0.75) -
      height * 0.1;
    return { x, y };
  });
...
const Experiment = () => {
  ...
  const [closeExperiment, setCloseExperiment] = useState<"yes" | "no" | null>(
    null,
  );
  ...
  const confirmCloseExperiment = async () => {
    if (!id || !detail?.experiment || detail.experiment.status === "completed")
      return;
  ...
  const avgIntensity =
    chartMetrics.length > 0
      ? chartMetrics.reduce((s, m) => s + m.intensity_value, 0) /
        chartMetrics.length
      : null;
  ...
  const intensityColorClass =
    avgIntensity === null ? "text-gray-500" : getIntensityColor(avgIntensity);
            ...
            <div className="relative h-[280px] md:h-[300px] w-full bg-[#0a0a1a]/50 rounded-2xl border border-white/10 overflow-hidden">
              ...
              <div className="absolute inset-0 grid grid-cols-6 md:grid-cols-12 grid-rows-4 md:grid-rows-6 opacity-40">
                {Array.from({ length: 24 }, (_, i) => (
                  <div
                    key={`grid-${i}`}
                    className="border-r border-b border-gray-800/30"
                  />
                ))}
              </div>

              <svg
                className="absolute inset-0 w-full h-full pr-8"
              ...
              <div className="flex flex-col md:flex-row gap-4 md:gap-6 items-start">
                ...
                <div className="flex flex-col gap-3 shrink-0 w-full md:w-auto">
                  <div className="flex flex-col md:flex-row items-start md:items-center gap-2 md:gap-4">
                    ...
                    <span className="text-lg md:text-xl">{successVal}/10</span>
                  </div>
                  <div className="flex flex-col md:flex-row items-start md:items-center gap-2 md:gap-4">
