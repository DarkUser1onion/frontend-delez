...
vi.mock("@/assets/Macbook Air M2 Silver Flatten.png", () => ({
  default: "macbook.png",
}));
...
describe("Landing", () => {
  ...
  it("renders how it works section", () => {
    renderLanding();
    expect(
      screen.getByText("Delёz запоминает, анализирует и связывает"),
    ).toBeInTheDocument();
  ...
  it("renders security section", () => {
    renderLanding();
    expect(
      screen.getByText("Твои данные принадлежат только тебе"),
    ).toBeInTheDocument();
  ...
  it("renders philosophy section", () => {
    renderLanding();
    expect(
      screen.getByText("Основано на концепциях Жиля Делёза"),
    ).toBeInTheDocument();
  ...
  it("renders CTA section with beta-test links", () => {
    renderLanding();
    const betaTestLinks = screen.getAllByRole("link", {
      name: "Записаться на бета-тест",
    });
