...
function GraphSelect({
  ...
  onChange,
  placeholder,
}: GraphSelectProps): React.JSX.Element {
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        ...
        >
          ...
          <ChevronDown
            size={13}
            className="ml-2 shrink-0 text-[#b6c2db]"
            strokeWidth={1.45}
          />
        </button>
      </DropdownMenuTrigger>
...
function toggleNodeType(activeTypes: Set<string>, type: string): Set<string> {
...
const GraphPage: React.FC = () => {
  ...
  const [isRelationsPanelVisible, setIsRelationsPanelVisible] =
  ...
  const [relationDeletingId, setRelationDeletingId] = useState<string | null>(
    null,
  );
  const [isRelationsListVisible, setIsRelationsListVisible] =
    useState<boolean>(false);
  const [isRelationHistoryVisible, setIsRelationHistoryVisible] =
    useState<boolean>(false);
  const [isNodeEditorVisible, setIsNodeEditorVisible] =
    useState<boolean>(false);
  ...
  const canCreateRelation = Boolean(
    relationFromId && relationToId && relationFromId !== relationToId,
  );
  ...
  useEffect(() => {
    ...
    setNodeDescription(
      typeof selectedNode.description === "string"
        ? selectedNode.description
        : "",
    );
    setNodeImageUrl(
      typeof selectedNode.image_url === "string" ? selectedNode.image_url : "",
    );
    setNodeTitle(
      typeof selectedNode.title === "string" ? selectedNode.title : "",
    );
  }, [selectedNode]);
  ...
  const removeRelationship = async (
    ...
    try {
      ...
      setRelationError(
        error_ instanceof Error ? error_.message : "Не удалось удалить связь.",
      );
    } finally {
            ...
            {NODE_TYPES.map((type) => (
              <button
                ...
                style={{
                  ...
                  border: activeTypes.has(type)
                    ? `1px solid ${NODE_COLORS[type]}aa`
                    : "1px solid rgba(255,255,255,0.14)",
                  boxShadow: activeTypes.has(type)
                    ? `0 0 14px ${NODE_COLORS[type]}55`
                    : "none",
                }}
              >
          ...
          <div className="mt-2 flex flex-col gap-2">
            {TIME_PERIODS.map((p) => (
              <button
                key={p.value}
                onClick={() =>
                  setTimePeriod((prev) => (prev === p.value ? null : p.value))
                }
                className="w-full rounded-xl px-3 py-1.5 text-left text-xs font-medium transition-all hover:-translate-y-[1px] focus:outline-none focus:ring-2 focus:ring-white/30"
                style={{
                  backgroundColor:
            ...
            <div className="mt-2 space-y-3">
              <div className="rounded-lg border border-white/10 bg-black/20 p-2">
                ...
                <p className="mt-1 text-[10px] text-white/55">
                  1) Источник {"->"} 2) Тип связи {"->"} 3) Цель {"->"} 4)
                  Сохранить
                </p>
              </div>
              ...
              <div className="rounded-lg border border-white/10 bg-black/20 p-2 space-y-2">
                <div>
                  <label
                    htmlFor="relation-from"
                    className="mb-1 block text-[11px] text-white/70"
                  >
                    Шаг 1. Узел источник
                  </label>
                  <GraphSelect
                ...
                {relationError ? (
                  <p className="text-[11px] text-rose-300">{relationError}</p>
                ) : null}
                {relationSuccess ? (
              ...
              <div className="rounded-lg border border-white/10 bg-black/20 p-2">
                ...
                >
                  ...
                  <span>
                    {isRelationHistoryVisible ? "Скрыть" : "Показать"}
                  </span>
                </button>
                      ...
                      relationHistory.slice(0, 8).map((item) => (
                        ...
                        >
                          <div className="flex-1 rounded-md border border-white/10 bg-white/5 px-2 py-1 text-[10px] text-white/75">
                            {RELATION_AUDIT_ACTION_LABELS[item.action] ??
                              item.action}
                            : {item.from_id.slice(0, 6)} {"->"}{" "}
                            {item.to_id.slice(0, 6)} (
                            {RELATIONSHIP_TYPE_LABELS[item.relationship] ??
                              item.relationship}
                            )
                          </div>
                          <button
                            type="button"
                            onClick={() =>
                              rollbackRelationshipChange(item.audit_id)
                            }
                ...
                {isNodeEditorVisible && (
                  <div className="mt-2 space-y-2">
                    <p className="text-[10px] text-white/45">
                      Выбран:{" "}
                      {selectedNode
                        ? `${NODE_TYPE_LABELS[selectedNode.type] ?? selectedNode.type} (${selectedNode.id})`
                        : "ничего"}
                    </p>
                    <input
                      value={nodeTitle}
                    ...
                    />
                    {nodeError ? (
                      <p className="text-[11px] text-rose-300">{nodeError}</p>
                    ) : null}
                    {nodeSuccess ? (
                      <p className="text-[11px] text-emerald-300">
                        {nodeSuccess}
      ...
      <div className="absolute inset-0 z-0 min-h-0 min-w-0">
        <ForceGraph3D
          ...
          nodeThreeObject={(node: unknown) => {
            ...
            const imageUrl =
              typeof graphNode.image_url === "string"
                ? graphNode.image_url
                : null;
            if (!imageUrl) {
              return new THREE.Object3D();
          ...
          linkPositionUpdate={(sprite, link) => {
            ...
            const middleX =
              coords.start.x + (coords.end.x - coords.start.x) / 2;
            const middleY =
              coords.start.y + (coords.end.y - coords.start.y) / 2;
            const middleZ =
              coords.start.z + (coords.end.z - coords.start.z) / 2;
