...
export default function SignIn() {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { response, data } = await authApi.signIn(
        requestBody.email,
      ...
      logger.authEvent("User signed in successfully", {
      ...
      setError(
        err?.message || "Не удалось выполнить вход. Попробуйте ещё раз.",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSignInError = async (response: Response, data: any) => {
    ...
    if (response.status === 500) {
      ...
      throw new Error(
        "Ошибка сервера. Пожалуйста, попробуйте позже или обратитесь в поддержку.",
      );
    }
    ...
    if (response.status === 401) {
      ...
      if (
        ...
        message.toLowerCase().includes("не подтвержден")
      ) {
    ...
    const hasVerifyMessage =
      message.toLowerCase().includes("verify") ||
      message.toLowerCase().includes("подтвер");
      ...
      <div className="auth-container">
        <div className="form-box">
          ...
          <p className="auth-subtitle">
            Войдите в аккаунт, чтобы продолжить путь
          </p>
                ...
                {isLoading ? (
                  <>
                    <Loader2
                      ...
                      aria-hidden="true"
                    />
                    <span id="loading-status" style={srOnlyStyle}>
                      Выполняется вход...
                    </span>
                  </>
                ) : (
          ...
          <div className="auth-link">
            Хотите попробовать?{" "}
            <Link to="/beta-test">Записаться на бета-тест</Link>
          </div>
        </div>
      </div>
    </div>
