...
vi.mock("@/components/PasswordToggleButton", () => ({
  default: ({ show, onToggle }: { show: boolean; onToggle: () => void }) => (
    <button
      onClick={onToggle}
      data-testid={`pwd-toggle-${show ? "hide" : "show"}`}
    >
      toggle
    </button>
  ),
...
describe("ResetPassword", () => {
  ...
  describe("without token", () => {
    it("shows invalid link message when no token in URL", () => {
      ...
      expect(
        screen.getByRole("heading", { name: TEXT.invalidLinkTitle }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("button", { name: TEXT.requestNewLink }),
      ).toBeInTheDocument();
    });
  ...
  describe("with token", () => {
    ...
    it("renders reset password form when token is present", () => {
      ...
      expect(
        screen.getByRole("heading", { name: TEXT.newPasswordTitle }),
      ).toBeInTheDocument();
      expect(screen.getByLabelText(TEXT.newPasswordLabel)).toBeInTheDocument();
      expect(
        screen.getByLabelText(TEXT.confirmPasswordLabel),
      ).toBeInTheDocument();
    });
    ...
    it("shows error when passwords do not match", async () => {
      ...
      fireEvent.submit(
        screen.getByLabelText(TEXT.newPasswordLabel).closest("form")!,
      );
    ...
    it("shows success screen after successful password reset", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByRole("heading", { name: TEXT.successTitle }),
        ).toBeInTheDocument();
      });
    });
    ...
    it("navigates immediately when sign-in button clicked on success screen", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByRole("button", { name: TEXT.signInNow }),
        ).toBeInTheDocument();
      });
    ...
    it("shows error when server returns non-ok response", async () => {
      vi.mocked(fetch).mockResolvedValueOnce(
        makeFetchResponse(400, { message: "Token expired" }),
      );
