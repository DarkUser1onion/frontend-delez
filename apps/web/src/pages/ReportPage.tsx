...
function DotChart({
  ...
  if (!data.length)
    return <div className="text-gray-600 text-xs">Нет данных</div>;
    ...
    <div className="flex items-end gap-1 h-24">
      {data.map((d) => {
        ...
        return (
          <div
            key={d.label}
            className="flex flex-col-reverse gap-[3px] items-center"
          >
...
export default function ReportPage() {
    ...
    Promise.all([
      ...
      .then(([entriesData, goalsData, graphData]) => {
        ...
        setStats({
          ...
          plannedGoals: goals.filter(
            (g) => g.status === "planned" || g.status === "paused",
          ).length,
          nodesByType,
          entriesByMonth,
        });
  ...
  const s = stats!;
  const goalPct = s.totalGoals
    ? Math.round((s.completedGoals / s.totalGoals) * 100)
    : 0;
  const activePct = s.totalGoals
    ? Math.round((s.activeGoals / s.totalGoals) * 100)
    : 0;
  ...
  const totalNodes =
    Object.values(s.nodesByType).reduce((a, b) => a + b, 0) || 1;
  const donutSegments = Object.entries(s.nodesByType).map(
    ([type, count], i) => ({
    ...
    <div className="min-h-full bg-[#000019] text-white">
      ...
      <div className="flex items-start justify-between px-8 pt-8 pb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm">
            Общее собирательное со всех разделов
          </p>
        </div>
        <button
      ...
      <div className="px-8 pb-16 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {/* Всего событий */}
          <div
            className="rounded-2xl p-5 col-span-1 sm:col-span-2 flex flex-col justify-between"
            style={{
          ...
          {/* Выполнение целей */}
          <div
            className="rounded-2xl p-5 flex flex-col items-center justify-between gap-3"
            style={{
              background: "rgba(255,255,255,0.04)",
              border: "1px solid rgba(255,255,255,0.08)",
            }}
