...
describe("ExperimentsPage", () => {
  ...
  it("показывает ошибку при сбое API", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Не удалось загрузить эксперименты"),
      ).toBeInTheDocument();
    });
  ...
  it("фильтрует по статусу active", async () => {
    ...
    expect(screen.getByText("Активный эксперимент")).toBeInTheDocument();
    expect(
      screen.queryByText("Завершённый эксперимент"),
  ...
  it('показывает "Нет экспериментов в этой категории" при пустом фильтре', async () => {
    ...
    expect(
      screen.getByText("Нет ветвлений в этой категории"),
    ).toBeInTheDocument();
  });
  ...
  it("рендерит статус paused", async () => {
    ...
    await waitFor(() =>
      expect(screen.getAllByText("На паузе").length).toBeGreaterThan(0),
    );
  });
